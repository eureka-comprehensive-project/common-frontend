    ...
    async function renderPlanTable(plans) {
        ...
        plans.forEach((plan) => {
            ...
            if (plan.dataAllowance === 99999) {
                dataDisplay = "무제한";
            } else if (plan.dataAllowance.dataAmount >= 10 && plan.dataAllowanceUnit) {
                dataDisplay = "대용량";
            }  else if (plan.dataAllowance.dataAmount < 10 && plan.dataAllowanceUnit) {
                dataDisplay = "소용량";
            } else {
        ...
        <td>
            <button class="btn-table-action modify-btn" data-plan-id="${
                plan.planId
            }">수정</button>
            </td>
      `;
    ...
    async function fetchAndRenderPlans(searchTerm = null, filters = null) {
        ...
        if (hasSearchTerm && !hasFilters) {
            ...
            const filterRequestDto = {
                planName: hasSearchTerm ? searchTerm : null,

                // 'categoryIds' 키로 변환된 ID 배열을 전송
                categoryIds: categoryIds,
                allCategoriesSelected: filters.category.length === 0,

                monthlyFeeRanges: filters.monthlyFee.length > 0 ? filters.monthlyFee : [],
                anyPriceSelected: filters.monthlyFee.length === 0,

                dataAllowanceTypes: filters.dataAllowance.length > 0 ? filters.dataAllowance : [],
            ...
            if (currentModalMode === "edit" && currentEditingPlanId) {
                ...
                try {
                    ...
                    if (!getResponse.ok) {
                        ...
                        throw new Error(
                            `현재 요금제 상태를 가져오지 못했습니다: ${
                                errorData.message || getResponse.statusText
                            }`
                        );
                    }
