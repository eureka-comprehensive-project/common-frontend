...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchForbiddenWords(searchValue = '', filterType = 'all', forceRefresh = false) {
        try {
            ...
            console.error('예외 금칙어 목록 조회 오류:', error);
            showToast('예외 금칙어 목록을 불러오는데 실패했습니다.', 'error');
            allForbiddenWords = [];
    ...
    /**
     * 필터링 및 검색 적용
     * @param {Array} words - 전체 예외 금칙어 목록
     * @param {string} searchValue - 검색어
     * @param {string} filterType - 필터 타입
     * @returns {Array} 필터링된 예외 금칙어 목록
     */
...
            });
            
            showToast('예외 금칙어가 성공적으로 추가되었습니다.', 'success');
            return true;
        } catch (error) {
            // 500 에러를 중복 등록으로 간주하고 특정 메시지 표시
            if (error && error.message && error.message.includes('500')) {
                showToast('이미 등록된 예외 금칙어 입니다.', 'error');
            } else {
                // 그 외 다른 에러는 기존 방식대로 표시
                const message = error.message || '예외 금칙어 추가에 실패했습니다.';
                showToast(`예외 금칙어 추가 실패: ${message}`, 'error');
            }
            return false;
        }
    }
    ...
    /**
     * 선택된 예외 금칙어 ID 목록 가져오기
     * @returns {Array} 선택된 예외 금칙어 ID 배열
     */
    ...
    async function bulkChangeWordStatus(targetStatus) {
        ...
        if (selectedIds.length === 0) {
            showToast('상태를 변경할 예외 금칙어를 선택해주세요.', 'warning');
            return;
        }
        
        const statusText = targetStatus ? '사용' : '미사용';
        showToast(`${selectedIds.length}개 예외 금칙어의 상태를 ${statusText}으로 변경 중...`, 'info');
        ...
        if (successCount > 0) {
            showToast(`${successCount}개의 예외 금칙어 상태가 ${statusText}으로 변경되었습니다.`, 'success');
        }
    }
    ...
    async function bulkDeleteWords() {
        const selectedIds = getSelectedWordIds();
        if (selectedIds.length === 0) {
            showToast('삭제할 예외 금칙어를 선택해주세요.', 'warning');
            return;
        }
        
        if (!confirm(`선택한 ${selectedIds.length}개의 예외 금칙어를 정말 삭제하시겠습니까?`)) {
            return;
        }
        
        showToast(`${selectedIds.length}개 예외 금칙어 삭제 중...`, 'info');
        ...
        if (successCount > 0) {
            showToast(`${successCount}개의 예외 금칙어가 삭제되었습니다.`, 'success');
    ...
    window.handleToggleStatus = async function(wordId, isChecked) {
        ...
        try {
            ...
            if (await toggleWordStatus(wordId)) {
                ...
                showToast(`예외 금칙어가 ${statusText}되었습니다.`, 'success');
            } else {
                // 실패: 토글 원래 상태로 복원
                if (toggleInput) {
                    toggleInput.checked = currentStatus;
                }
                showToast('예외 금칙어 상태 변경에 실패했습니다.', 'error');
            }
        } catch (error) {
            // 오류: 토글 원래 상태로 복원
            if (toggleInput) {
            ...
            showToast('예외 금칙어 상태 변경 중 오류가 발생했습니다.', 'error');
        } finally {
            // 토글 버튼 다시 활성화
            if (toggleInput) {
    ...
    window.handleDeleteWord = async function(wordId) {
        if (!confirm('이 예외 금칙어를 정말 삭제하시겠습니까?')) {
            return;
        }
        ...
        try {
            if (await deleteForbiddenWord(wordId)) {
                ...
                showToast('예외 금칙어가 삭제되었습니다.', 'success');
                updateStatistics();
            } else {
                showToast('예외 금칙어 삭제에 실패했습니다.', 'error');
            }
        } catch (error) {
            showToast('예외 금칙어 삭제 중 오류가 발생했습니다.', 'error');
        } finally {
    ...
    elements.modalRegisterBtn.addEventListener('click', async () => {
        ...
        if (!word) {
            showToast('예외 금칙어를 입력해주세요.', 'warning');
            elements.modalWordInput.focus();
            return;
        }
     ...
     */
    function initialize() {
        console.log('예외 금칙어 관리 페이지 초기화 시작');
        
        // 초기 데이터 로드
        fetchForbiddenWords('', 'all', true);
        
        console.log('예외 금칙어 관리 페이지 초기화 완료');
    }
